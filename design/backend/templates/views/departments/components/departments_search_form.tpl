...
<form action="{""|fn_url}{$_page_part}" name="{$department_search_form_prefix}search_form" method="get" class="cm-disable-empty {$form_meta}" id="search_form">
...
{capture name="simple_search"}
    {hook name="departments:simple_search"}
...
{hook name="departments:advanced_search"}
    ...
    <div class="table-wrapper">
        <table width="100%">
            <tr class="nowrap">
                <td><label for="pname" class="checkbox inline"><input type="checkbox" value="Y" {if $search.pname == "Y"}checked="checked"{/if} name="pname" id="pname" />{__("department_name")}</label></td>
...
{if $filter_items}
    <div class="control-group">
        ...
        {__("search_by_department_filters")}</a>

        <div id="filter"{if !$smarty.cookies.filter} class="hidden"{/if}>
            {include file="views/departments/components/advanced_search_form.tpl" filter_features=$filter_items prefix="filter_" data_name="filter_variants"}
        </div>
    </div>
{/if}
</div>
...
<div class="group span6 form-horizontal">
    ...
    {hook name="companies:departments_advanced_search"}
    ...
    <div class="group span6 form-horizontal">
        ...
        <div class="control-group">
            ...
            <select name="status" id="status">
                <option value="">--</option>
                {foreach fn_get_departments() as $status_id => $status_name}
                ...
                {** Hook for additional fields in the departments search form *}
        {hook name="departments:search_form"}{/hook}
    </div>

    <div class="group span6 form-horizontal">
        {** The 'Search in orders' field hook *}
        {hook name="departments:search_in_orders"}
        <div class="control-group">
        ...
        <div class="control-group">
            ...
            <select class="select-mini" name="sort_by" id="sort_by">
                ...
                <option {if $search.sort_by == "department"}selected="selected"{/if} value="department">{__("name")}</option>
                ...
                {hook name="departments:select_search"}
                {/hook}
            </select> -
...
<div class="group form-horizontal">
    <div class="control-group">
        ...
        <div class="controls">
            {include file="common/period_selector.tpl" period=$search.period form_name="{$department_search_form_prefix}search_form"}
        </div>
    </div>
</div>
...
{include file="common/advanced_search.tpl" simple_search=$smarty.capture.simple_search advanced_search=$smarty.capture.advanced_search dispatch=$dispatch view_type="departments" in_popup=$in_popup is_order_management=$is_order_management}
