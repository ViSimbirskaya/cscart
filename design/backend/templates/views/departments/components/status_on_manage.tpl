{$items_status = $items_status|default:($status|fn_get_departmes:$hidden)}
...
{hook name="departments:status_name_container"}
{if $non_editable || $display == "text"}
    <span class="view-status">
        {hook name="departments:status_name"}
...
{else}
    ...
    <div class="cm-popup-box {if !$hide_for_vendor}dropdown{/if} dropleft {$popup_additional_class}"
        id="department_status_{$id}_select">
        {hook name="departments:status_name"}
        {if !$hide_for_vendor}
            <a href="#"
        ...
        {if $id && !$hide_for_vendor}
            <ul class="dropdown-menu">
                ...
                {foreach $items_status as $status_id => $status_name}
                    <li {if $status == $status_id}class="disabled"{/if}>
                        {hook name="departments:status_select_item"}
            ...
            {/if}
        {/if}
    <!--department_status_{$id}_select--></div>
{/if}
{/hook}
